...
use super::legacy::{LocationIndex, PoloniusLocationTable};
use crate::borrow_set::BorrowSet;
use crate::constraints::OutlivesConstraint;
use crate::path_utils::*;
use crate::{
    AccessDepth, Activation, ArtificialField, BorrowIndex, Deep, LocalMutationIsAllowed, Read,
    ReadKind, ReadOrWrite, RegionInferenceContext, Reservation, Shallow, Write, WriteKind,
};

pub(super) fn compute_relevant_outlives_constraints<'tcx>(
    tcx: TyCtxt<'tcx>,
    ...
    let loan_invalidations = compute_loan_invalidations(tcx, body, location_table, borrow_set);
    let loan_invalidation_constraints = compute_relevant_outlives_constraints_for_borrows(
        regioncx,
        borrow_set,
        loan_invalidations.into_iter().map(|(_, x)| x),
    );
    let all_borrows_constraints = compute_relevant_outlives_constraints_for_borrows(
        regioncx,
        borrow_set,
        borrow_set.indices(),
    );
    debug!(no_loan_invalidation_constraints = loan_invalidation_constraints.len());
    debug!(no_all_borrows_constraints = all_borrows_constraints.len());
    debug!(no_all_constraints = regioncx.outlives_constraints().count());
    if let Ok(mut f) = std::fs::File::create_new("f.txt") {
        use std::io::Write as _;
        writeln!(f, "--- Borrows ---").unwrap();
        for (_, borrow) in borrow_set.iter_enumerated() {
            writeln!(f, "{borrow}").unwrap();
        }
        writeln!(f, "\n--- Loan Invalidation Constraints ---").unwrap();
        for constraint in loan_invalidation_constraints.iter() {
            writeln!(
                f,
                "{}: {}",
                regioncx.region_value_str(constraint.sup),
                regioncx.region_value_str(constraint.sub)
            )
            .unwrap();
        }
        writeln!(f, "\n--- All Borrows Constraints ---").unwrap();
        for constraint in all_borrows_constraints.iter() {
            writeln!(
                f,
                "{}: {}",
                regioncx.region_value_str(constraint.sup),
    ...
    loan_invalidation_constraints
}

fn compute_relevant_outlives_constraints_for_borrows<'tcx>(
    regioncx: &RegionInferenceContext<'tcx>,
    borrow_set: &BorrowSet<'tcx>,
    borrows: impl IntoIterator<Item = BorrowIndex>,
) -> Vec<OutlivesConstraint<'tcx>> {
    let mut result = Vec::new();
    let mut to_check =
        DenseBitSet::new_empty(regioncx.last_region_vid().map_or(0, |x| x.index() + 1));
    let mut to_check_next_round = to_check.clone();
    for borrow_idx in borrows {
        to_check.insert(borrow_set[borrow_idx].region());
    }
    let mut visited = to_check.clone();
    let mut reached_fix_point = false;
    while !reached_fix_point {
        reached_fix_point = true;
        for constraint in regioncx.outlives_constraints() {
            if to_check.contains(constraint.sup) {
                result.push(constraint);
                reached_fix_point = false;
    ...
    // stack is a subset of visited. That is, regions are only added to stack if they are not
    // members of visited.

    let mut result = Vec::new();

    for borrow_idx in borrows {
        let region = borrow_set[borrow_idx].region();
        ...
        while let Some(region) = stack.pop() {
            for constraint in regioncx.outgoing_outlives_constraints(region) {
                assert_eq!(region, constraint.sup);
                result.push(constraint);
                if !visited.insert(constraint.sub) {
...
fn compute_loan_invalidations<'tcx>(
    tcx: TyCtxt<'tcx>,
    body: &Body<'tcx>,
    location_table: &PoloniusLocationTable,
    borrow_set: &BorrowSet<'tcx>,
) -> Vec<(LocationIndex, BorrowIndex)> {
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        ...
        match &terminator.kind {
            ...
            TerminatorKind::Yield { value, resume, resume_arg, drop: _ } => {
                ...
                for (i, data) in borrow_set.iter_enumerated() {
                    if borrow_of_local_data(data.borrowed_place) {
                        self.loan_invalidated_at.push((resume, i));
                    }
                }
            ...
            | TerminatorKind::CoroutineDrop => {
                ...
                for (i, data) in borrow_set.iter_enumerated() {
                    if borrow_of_local_data(data.borrowed_place) {
                        self.loan_invalidated_at.push((start, i));
                    }
                }
            }
...
impl<'a, 'tcx> LoanInvalidationsGenerator<'a, 'tcx> {
    ...
    fn emit_loan_invalidated_at(&mut self, b: BorrowIndex, l: Location) {
        ...
        self.loan_invalidated_at.push((lidx, b));
    }
